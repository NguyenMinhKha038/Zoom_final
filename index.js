...
var storageimg = multer.diskStorage({
	destination:function(req,file,cb){
		cb(null,`./public/labeled_images/${req.session.ClassID}/${NameofStudent}`)
	},
	filename:function(req,file,cb){
		cb(null,file.originalname)
	}
...
app.post('/create/classs', checkSession, (req, res) => {
	...
	Class.create({ Name: req.body.Name, ClassName: req.body.ClassName, TeacherID: req.session.TeacherID }, (err, classs) => {
		...
		fs.mkdirSync(`./public/labeled_images/${classs._id}`)
	})
})
...
app.get('/deleteClass/:id', (req, res) => {
	...
	Class.findById(req.params.id, function (error, detailClass) {
		...
		fs.rmdirSync(`./public/labeled_images/${req.session.ClassID}`, { recursive: true });
		res.redirect('/viewClass');
...
app.get('/importhinh',checkSession, (req, res) => {
	res.render('importPic');
})
app.get('/diemdanh',checkSession, (req, res) => {
	res.render('diemdanh' ,{
		studenname : NameofStudent,
		classId : req.session.ClassID
	});
})
...
io.on('connection', (socket) => {
	socket.emit('name-id', ClassIDofStudent,NameofStudent);
	socket.on('reckonized', (tenhs) => {
	Tentrave = tenhs
	})
